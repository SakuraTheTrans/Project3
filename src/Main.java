...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Task task1 = new Task("Fix Login Bug", "Resolve issue where users cannot log in with special characters.", 3);
        Task task2 = new Task("Update UI", "Redesign the dashboard for a more modern look.", 3);
        Task task3 = new Task("Write Documentation", "Complete API documentation for the new authentication system.", 5);
        Task task4 = new Task("Optimize Database", "Improve query performance by adding necessary indexes.", 5);
        Task task5 = new Task("Refactor Code", "Clean up redundant methods and improve maintainability.", 1);

        taskCollector.addTask(task1);

        taskCollector.addTask(task2);

        taskCollector.addTask(task3);

        taskCollector.addTask(task4);

        taskCollector.addTask(task5);
